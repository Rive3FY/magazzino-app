...
export default function MovimentiPage() {
  ...
  /** Flusso guidato uscita materiali: 1 = Materiale, 2 = Dettagli e conferma (solo quando type === "OUT") */
...
async function loadMaterialForScan(code: string, wh: "PRM" | "REALE"): Promise<QuickMaterialInfo | null> {
  ...
  async function addRemotePickToCart(event: RemoteScanEvent) {
    ...
    if (scanMode === "NORMAL") {
      setType("OUT");
      setWarehouse(wh);
      setPicked({ code, name: info.name, um: info.um });
      setSearch(`${code} — ${info.name}`);
      setQty(String(qtyPick));
      setOpen(false);
      setCartOpen(false);
      setMsg(null);
...
function finalizeMaterialPickupSuccess() {
      ...
      try {
        ...
        const movementsPayload = closingGroup.map((mov) => {
          ...
          return {
            ...
            warehouse: mov.warehouse,
            created_at: mov.created_at ?? null,
            out_qty: outQty,
            returned_qty: r,
            net_qty: Math.max(0, outQty - r),
    ...
    try {
      const res = await fetch("/api/send-close-email", {
        ...
        body: JSON.stringify({
          ...
          warehouse: closing.warehouse,
          created_at: closing.created_at ?? null,
          out_qty: outQty,
          returned_qty: r,
          net_qty: Math.max(0, outQty - r),
  ...
  return (
    <main className="panel" style={{ overflowX: "hidden" }}>
      ...
      {/* Popup wizard prelievo materiali (solo uscita) */}
      {type === "OUT" && wizardMatOpen && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="wizard-mat-title"
          style={{
            position: "fixed",
            inset: 0,
          ...
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setWizardMatOpen(false);
          }}
        >
          <div
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 16 }}>
              <h2 id="wizard-mat-title" style={{ margin: 0, fontSize: 18, fontWeight: 900 }}>
                Prelievo · Passo {outboundStepMat}/2
              </h2>
              <button
                type="button"
                className="btn"
                onClick={() => setWizardMatOpen(false)}
                aria-label="Chiudi"
            ...
            {outboundStepMat === 1 && (
              <>
                ...
                {remoteMaterialScanOpen || remoteScanCode ? (
                  <div
                    style={{
                      marginBottom: 12,
                      padding: 12,
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                      <div>
                        ...
                        <div style={{ marginTop: 3, fontSize: 12, color: "#64748b" }}>
                          {remoteScanCode
                            ? `Sessione attiva in modalità ${scanMode === "CART" ? "carrello" : "singolo"} finché la chiudi tu.`
