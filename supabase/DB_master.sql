...
-- -----------------------------------------------------------------------------
-- 22A. RPC - statistiche dashboard materiali (solo admin materiali)
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'assigned_rows_total', (
      SELECT COUNT(*) FROM public.excel_live e
      INNER JOIN public.material_shelves ms ON ms.code = e.code AND ms.warehouse = e.warehouse
      WHERE NULLIF(btrim(ms.shelf), '') IS NOT NULL OR NULLIF(btrim(COALESCE(ms.place, '')), '') IS NOT NULL
    ),
    'assigned_rows_prm', (
      SELECT COUNT(*) FROM public.excel_live e
      INNER JOIN public.material_shelves ms ON ms.code = e.code AND ms.warehouse = e.warehouse
      WHERE e.warehouse = 'PRM'
        AND (NULLIF(btrim(ms.shelf), '') IS NOT NULL OR NULLIF(btrim(COALESCE(ms.place, '')), '') IS NOT NULL)
    ),
    'assigned_rows_reale', (
      SELECT COUNT(*) FROM public.excel_live e
      INNER JOIN public.material_shelves ms ON ms.code = e.code AND ms.warehouse = e.warehouse
    ...
    'orphan_shelf_rows', (
      SELECT COUNT(*) FROM public.material_shelves ms
      LEFT JOIN public.excel_live e ON e.code = ms.code AND e.warehouse = ms.warehouse
      WHERE e.code IS NULL
        AND (NULLIF(btrim(ms.shelf), '') IS NOT NULL OR NULLIF(btrim(COALESCE(ms.place, '')), '') IS NOT NULL)
    ),
    'excel_codes_without_item', (
      SELECT COUNT(DISTINCT e.code) FROM public.excel_live e LEFT JOIN public.items i ON i.code = e.code WHERE i.code IS NULL
    ),
    'zero_stock_count', (
      SELECT COUNT(*) FROM (
        SELECT code FROM public.excel_live GROUP BY code HAVING COALESCE(SUM(qty_free), 0) = 0
      ) z
    ),
    'zero_stock_prm', (SELECT COUNT(*) FROM public.excel_live WHERE warehouse = 'PRM' AND COALESCE(qty_free, 0) = 0),
    'zero_stock_reale', (SELECT COUNT(*) FROM public.excel_live WHERE warehouse = 'REALE' AND COALESCE(qty_free, 0) = 0),
    'negative_stock_count', (SELECT COUNT(*) FROM public.excel_live WHERE COALESCE(qty_free, 0) < 0),
    'open_out_movements', (SELECT COUNT(*) FROM public.movements WHERE type = 'OUT' AND status = 'OPEN'),
    'pending_excel_requests', (SELECT COUNT(*) FROM public.excel_live_requests WHERE status = 'pending'),
    'movements_today', (
...
CREATE OR REPLACE FUNCTION public.giacenze_list(p_view text, p_search text, p_sort_key text, p_sort_dir text, p_shelf_filter text, p_stock_filter text, p_limit int, p_offset int)
...
BEGIN
  ...
  LEFT JOIN public.material_shelves ms ON ms.code = e.code AND ms.warehouse = e.warehouse
  LEFT JOIN public.items i ON i.code = e.code
  WHERE e.warehouse = ANY(v_wh)
    AND (
      COALESCE(p_stock_filter, 'ALL') NOT IN ('ZERO', 'ZERO_TOTAL')
      OR (p_stock_filter = 'ZERO' AND COALESCE(e.qty_free, 0) = 0)
      OR (
        p_stock_filter = 'ZERO_TOTAL'
        AND e.code IN (
          SELECT z.code FROM public.excel_live z GROUP BY z.code HAVING COALESCE(SUM(z.qty_free), 0) = 0
        )
      )
    )
    AND (
      COALESCE(p_shelf_filter, 'ALL') NOT IN ('WITH','WITHOUT')
      OR (p_shelf_filter = 'WITH' AND ms.code IS NOT NULL AND (NULLIF(btrim(COALESCE(ms.shelf,'')), '') IS NOT NULL OR NULLIF(btrim(COALESCE(ms.place,'')), '') IS NOT NULL))
      OR (p_shelf_filter = 'WITHOUT' AND (ms.code IS NULL OR (NULLIF(btrim(COALESCE(ms.shelf,'')), '') IS NULL AND NULLIF(btrim(COALESCE(ms.place,'')), '') IS NULL)))
    )
    AND (p_search IS NULL OR p_search = '' OR e.code ILIKE '%'||p_search||'%' OR i.name ILIKE '%'||p_search||'%' OR COALESCE(e.row_json->>'Descrizione Materiale','') ILIKE '%'||p_search||'%');
  RETURN QUERY
