-- Q.tà 0 totale = solo codici con somma PRM+REALE a zero.
-- Q.tà 0 PRM/REALE restano per singola posizione magazzino.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'zero_stock_count', (
      ...
      FROM (
        SELECT code
        FROM public.excel_live
        GROUP BY code
        HAVING COALESCE(SUM(qty_free), 0) = 0
      ) z
    ),
    'zero_stock_prm', (
...
BEGIN
  ...
  WHERE e.warehouse = ANY (v_wh)
    AND (
      ...
      OR (
        p_stock_filter = 'ZERO'
        AND COALESCE(e.qty_free, 0) = 0
      )
      OR (
        p_stock_filter = 'ZERO_TOTAL'
        AND e.code IN (
          SELECT z.code
          FROM public.excel_live z
          GROUP BY z.code
          HAVING COALESCE(SUM(z.qty_free), 0) = 0
        )
      )
    )
    AND (
